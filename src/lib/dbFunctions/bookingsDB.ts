...
export async function deleteBookingByID(supabase: SupabaseClient, venueID: string | undefined, bookingID: string | undefined): Promise<DBResult<any>> {
    const missing = ensureArgs({ p_venue_id: venueID, p_booking_id: bookingID })
    if (missing) return { data: null, error: missing };
